...
class boringen():
    ...
    def get_boringen(self,folder, ds):
        ...
        for file in tqdm(csv_files, 'Read borehole files..'):
            ...
            if (md.loc['X-coordinaat (m)'].values[0] >= ds.extent[0]) and (md.loc['X-coordinaat (m)'].values[0] <=  ds.extent[1]):
                if (md.loc['X-coordinaat (m)'].values[0] >= ds.extent[2]) and (md.loc['X-coordinaat (m)'].values[0] <=  ds.extent[3]):
