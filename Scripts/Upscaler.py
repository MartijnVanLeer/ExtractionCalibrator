...
for cellid in ids:
    ...
    for sim in range(ens_no):
        fieldK = uf.Run_MF_WholeField(cellk[f'K_{sim +1}'].values,Lx =2*dx,Ly = 2*dx,Lz = cellk.z.max() - cellk.z.min(),dx = real_dx,dy = real_dx,dz = 1, mds = mds, model_name = model_name)
